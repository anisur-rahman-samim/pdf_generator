...
export default function Home() {
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(`https://pdf-qrcode-backend.vercel.app/download/${idNumberVal}`, {
      width: 200,
      margin: 0,
  ...
  const downloadPdfSnapshot = async () => {
    toast.loading("Downloading....", {id:"555"});
    if (!previewRef.current) return toast.error("ref not found", {id:"555", duration:3000});;
...
formData.append(
    "pdf",
    pdfBlob,
    `id-card-${watch("idNumber") || "preview"}.pdf`
  );
  formData.append("idNumber", watch("idNumber"))

   try {
    const res = await fetch("https://pdf-qrcode-backend.vercel.app/upload", {
    method: "POST",
    body: formData,
  });
  ...
  const data = await res.json()
    if(data.success){ 
      ...
      toast.success(data?.message, {id:"555", duration:3000})
    }else{
      toast.error(data?.message ?? "pdf generate failed", {id:"555", duration:3000})
    }
   } catch (error:any) {
    toast.error(error?.message ?? "pdf generate failed", {id:"555", duration:3000})
   }
